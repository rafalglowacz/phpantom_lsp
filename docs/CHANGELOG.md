...
- **Mago diagnostic proxy.** Mago lint and analyze diagnostics are surfaced as LSP diagnostics with quick-fix code actions. When `mago.toml` exists at the workspace root and the `mago` binary is available (via Composer bin-dir or `$PATH`), both `mago lint` (fast, AST-level rules) and `mago analyze` (slower, type-aware analysis) run in dedicated background workers with debounce and cancellation. Issues that carry fix edits are offered as quick-fix code actions; safe fixes are marked as preferred, while potentially-unsafe and unsafe fixes are labelled accordingly. Configurable under `[mago]` in `.phpantom.toml` with `command`, `lint-timeout`, and `analyze-timeout` options. Requires Mago 1.15+.
...
- **Closure inlay hints pass call-site argument text for template substitution.** When a function has `@template T` and a callable parameter typed `callable(T): T`, the inlay hint system now passes the full argument text from the enclosing call site through to the template inference engine. This is the infrastructure step toward showing concrete types (e.g. `int`) instead of raw template names for closure parameters when the type can be inferred from sibling arguments.
- **Closure and arrow function inlay hints.** When a closure or arrow function is passed to a callable-typed parameter, inlay hints show inferred parameter types before untyped `$param` variables and the return type after the closing parenthesis. Types are derived from the enclosing callable signature (e.g. `fn($u) => $u->name` passed to a `Closure(User): string` parameter shows `User` before `$u` and `: string` after the parens). Fully qualified names are shortened to their base name. Hints are suppressed for `mixed` types and parameters that already have type declarations.
- **Unused variable diagnostics.** Variables assigned but never read are flagged with hint severity and rendered as dimmed text. Covers local variables, foreach key/value bindings, catch variables, and closure `use` bindings. Variables named `$_` or prefixed with `$_` are exempt (intentional discard). Catch variables are only flagged when the target PHP version is 8.0 or newer (older versions require the variable in `catch` syntax). Parameters are intentionally not flagged until suppression support is available.
- **Convert to instance variable.** A code action that promotes a local variable inside a method to a class property. All references within the method are rewritten to `$this->prop` (or `self::$prop` in static methods). The action is suppressed when a property with the same name already exists or when the cursor is on `$this`.
...
- **Use-imported classes no longer shadowed by global-namespace stubs.** When a file had `use Illuminate\Support\Facades\Event;` and called `Event::listen(...)`, the class loader found the PECL `Event` stub (global namespace) before consulting the use-map, producing false "method not found" diagnostics. The use-map is now checked first for unqualified names, fixing Facade static method resolution and eliminating ~70 false positives across test projects.
...
- **Closure parameter `@param` annotations no longer leak across sibling closures.** When two closures in the same method used the same parameter name (e.g. `$row`), the `@param` docblock from the first closure could incorrectly override the native type hint of the second. The docblock scanner now verifies the annotation is directly attached to its closure before applying it.
- **Conditional return types with concrete `class-string` bounds.** Methods with conditional return types like `($type is class-string<SomeInterface> ? ThenType : ElseType)` now check whether the argument class actually implements the bound interface before choosing the return branch. Previously any `::class` argument unconditionally took the then-branch, producing wrong types (e.g. Symfony's `createForm()` returning the form type class instead of `FormInterface`).
- **Conditional return type names resolved across files.** Class names in `@return` conditional type annotations (e.g. `class-string<FormFlowTypeInterface>`) are now resolved through the defining file's `use` statements, fixing cross-file conditional return type evaluation where the bound class was imported only in the vendor file.
